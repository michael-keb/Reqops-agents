...
Late-binding delegates so ``unittest.mock.patch('api.providers.modal.volume_X')``
keeps working — see the daytona adapter for the rationale. Modal does
NOT expose a ``volume_download`` primitive — the inherited base default
delegates to ``read``.
"""
