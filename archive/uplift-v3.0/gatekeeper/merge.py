"""Merge fresh classification with prior grid — X3/X5 durability (KeepLock)."""
...
from gatekeeper.legend import CANONICAL_GAPS
from gatekeeper.models import AnswerHistory, Exposure, GapRow, StateGrid

LOCKED_EXPOSURES: frozenset[str] = frozenset({"X3", "X5"})

# gap, pattern that must appear in locked_by (or lock context), pattern in new message
...
def directly_negates(new_message: str, locked_by: str | None, gap: str) -> bool:
    """True only when latest text explicitly reverses the locked fact."""
    ...
    if _EXPLICIT_REOPEN.search(new_message):
        for gap_code, pat_lock, pat_new in _GAP_NEGATION_RULES:
            ...
            if pat_lock.search(locked) and pat_new.search(new_message):
                return True
        # Explicit reversal language without a gap rule — only for user-base / scope gaps
...
def _reopen_row(prior: GapRow, fresh: GapRow, turn: int) -> GapRow:
    return replace(
        fresh,
        exposure="X6",
        locked_by=None,
...
def merge_gap(
    ...
    *,
    ...
    """KeepLock: prior X3/X5 wins unless direct negation or classifier X6 reopen."""
    if prior is None:
    ...
    if prior.exposure in LOCKED_EXPOSURES:
        ...
        if negated or fresh.exposure == "X6":
            return _reopen_row(prior, fresh, turn)
